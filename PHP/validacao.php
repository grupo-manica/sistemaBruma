...
$nome       = trim($_POST['nome1']    ?? $_POST['nome']    ?? '');
$cpf        = trim($_POST['cpf1']     ?? $_POST['cpf']     ?? '');
...
$tipo       = trim($_POST['escolha'] ?? $_POST['tipo'] ?? ''); // Cliente / Profissional
$email      = trim($_POST['email']   ?? $_POST['email'] ?? '');
$login_in   = trim($_POST['login']    ?? $_POST['email1'] ?? ''); // usado no login
$senha      = $_POST['senha'] ?? '';
...
if ($is_register) {
    ...
    // Tipo (escolha1) — normaliza para minúsculas
    $tipoLower = mb_strtolower($tipo);
    if ($tipoLower !== 'cliente' && $tipoLower !== 'profissional') {
        $errors[] = "Selecione o tipo de usuário (Cliente ou Profissional).";
    }
    ...
    $_SESSION['nome']  = $nome;
    $_SESSION['role']  = $tipoLower === 'cliente' ? 'cliente' : 'profissional';
